meta:
  ...
  bit-endian: le
  imports:
    - standard_midi_file_with_running_status_write
...
instances:
  ...
  is_mpc2000xl_mid:
    value: tracks.size > 0 and meta_events.size > 0 and
           ...
           first_meta.body.to_s("ASCII").substring(0, 16) == "MPC2000XL 1.00  "
  sequence_name:
...
seq:
  - id: hdr
    type: standard_midi_file_with_running_status_write::header
  - id: tracks
    type: standard_midi_file_with_running_status_write::track
    repeat: expr
