// Hand-written smoke test for bitfield generated C++ read-write fixtures.

#include <boost/test/unit_test.hpp>
#include <sstream>

#ifdef KS_HAS_RW_BITS_WRITE
#include "bits_write.h"

BOOST_AUTO_TEST_CASE(test_bits_write_roundtrip_and_bytes) {
    std::stringstream io(std::ios::in | std::ios::out | std::ios::binary);
    kaitai::kstream ks_write(&io);
    bits_write_t writer(&ks_write);
    ...
    BOOST_CHECK_EQUAL(io.str(), std::string("\xda\xaa\x9b\xaa\x7e", 5));

    io.seekg(0);
    kaitai::kstream ks_read(&io);
    bits_write_t reader(&ks_read);
    reader._read();
...
#else
BOOST_AUTO_TEST_CASE(test_bits_write_roundtrip_and_bytes) {
    BOOST_FAIL("bits_write read-write fixture was not generated; run tests/build-formats cpp_stl_11 first");
}
#endif
